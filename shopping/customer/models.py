...
def create_invoice(sender, instance, created, **kwargs):
    """
    Automatically creates an invoice when an order is marked as complete,
    if an invoice does not already exist for the order.
    """
    if instance.complete and created:
        # Check if an invoice already exists for this order
        existing_invoice = Invoice.objects.filter(order=instance).exists()
